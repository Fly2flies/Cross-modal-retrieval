...
## Model path ##
ROOT_PATH = 'saved/S2'
Image_Generator_Path = os.path.join(ROOT_PATH, 'imgcnn_epoch25.pth')
Text_Generator_Path = os.path.join(ROOT_PATH, 'textcnn_epoch25.pth')
Identity_Classifier_Path = os.path.join(ROOT_PATH, 'id_classifier_epoch25.pth')
...
for epoch in range(epochs):
    ...
    for i,batch in enumerate(train_dataloader):
        ...
        #predicted_second_image_modality = mode_classifier(second_image_features)

        predicted_first_text_modality = mode_classifier(first_text_features)
        #predicted_second_text_modality = mode_classifier(second_text_features)

        if epoch % K:
            # update Generator
            image_modality_loss = criterion_modality(predicted_first_image_modality, modality_text)
            text_modality_loss = criterion_modality(predicted_first_text_modality, modality_image)
        else:
