...
class Logger():
    def __init__(self, n_epochs, batches_epoch,eval_epochs, has_epoch = 1):
        self.viz = Visdom()
        self.n_epochs = n_epochs
        self.batches_epoch = batches_epoch
        self.epoch = has_epoch
        self.batch = 1
    ...
    def log(self,
        ...
        for i, loss_name in enumerate(losses.keys()):
            if loss_name not in self.losses:
                self.losses[loss_name] = losses[loss_name].data.cpu()
            else:
                self.losses[loss_name] += losses[loss_name].data.cpu()
        ...
        if (self.batch % self.batches_epoch) == 0:
            ...
            if (self.epoch % self.eval_epochs[0]) == 0:
                ...
                for metric_name, metric in self.metrics.items():
                    if metric_name not in self.metric_windows:
                        self.metric_windows[metric_name] = self.viz.line(X=np.array([self.epoch]),
                                                                     Y=np.array([metric]),
                                                                     opts={'xlabel': 'epochs',
                                                                           'ylabel': metric_name,
                                                                           'title': metric_name})
                    else:
                        self.viz.line(X=np.array([self.epoch]), Y=np.array([metric]),
